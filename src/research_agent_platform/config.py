...
class AppConfig(BaseModel):
    ...
    # Default completion budget for free-form text generation. Reasoning models (e.g. GLM-5.x)
    # emit a long ``reasoning_content`` first; when ``max_tokens`` is omitted the endpoint's small
    # default cap (often 4096) is consumed by reasoning and the visible ``content`` returns empty
    # or truncated. Setting an explicit budget lets the answer be emitted after reasoning. Callers
    # that already pass ``max_tokens`` (e.g. screening) are unaffected.
    upstream_max_output_tokens: int = Field(
        default_factory=lambda: int(os.getenv("UPSTREAM_MAX_OUTPUT_TOKENS", "32768"))
    )
    ...
    # Cap on candidates handed to the first-round (discovery) screening LLM call.
    review_screen_max_candidates: int = Field(
        default_factory=lambda: int(os.getenv("REVIEW_SCREEN_MAX_CANDIDATES", "50"))
    )
    # Cap on candidates handed to the second-round (focused) screening LLM call.
    review_focused_screen_max_candidates: int = Field(
        default_factory=lambda: int(os.getenv("REVIEW_FOCUSED_SCREEN_MAX_CANDIDATES", "30"))
    )
    ...
    # Independent screening batches run concurrently up to this fan-out. Wall-clock scales down by
    # roughly this factor; keep it small enough to stay under the upstream LLM gateway's rate limit.
    review_screen_concurrency: int = Field(
        default_factory=lambda: int(os.getenv("REVIEW_SCREEN_CONCURRENCY", "4"))
    )
    ...
    # First-round retrieval retain cap (wide recall pool before LLM screening).
    review_discovery_max_papers: int = Field(
        default_factory=lambda: int(os.getenv("REVIEW_DISCOVERY_MAX_PAPERS", "70"))
    )
    # Max papers fed to direction clustering (screened keep only; no longer a retrieval cap).
    review_direction_max_candidates: int = Field(
        default_factory=lambda: int(os.getenv("REVIEW_DIRECTION_MAX_CANDIDATES", "30"))
    )
    ...
    # Semantic Scholar Graph API key. The anonymous tier draws from a pool shared with
    # every other unauthenticated caller, so it returns HTTP 429 under contention no matter
    # how the client paces itself; a key moves the caller to a private ~1 req/s budget.
