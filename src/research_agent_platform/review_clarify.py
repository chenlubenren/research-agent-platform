...
def _merge_scope_card(topic: str, overlay: object) -> dict[str, str]:
    card = default_scope_card(topic)
    if not isinstance(overlay, dict):
        return card
    for key, value in overlay.items():
        name = str(key).strip()
        text = str(value).strip()
        if name in card and text:
            card[name] = text
    return card


async def build_clarify_draft(
    topic: str, *, generate_text: GenerateText, model: str | None = None
) -> tuple[list[str], dict[str, str]]:
    """One LLM call: topic-adaptive questions plus a prefilled default scope card."""

    keys = ", ".join(default_scope_card(topic).keys())
    system = (
        ...
        "questions tailored to the topic (merge/rewrite dimensions; do not ask them verbatim; pin down "
        "the scenario). Also prefill a default retrieval scope card a researcher could accept without "
        "answering: name concrete nearby concepts to include/exclude, likely time cut, and representative "
        "benchmarks or methods for THIS topic (e.g. well-known datasets or systems), not generic filler.\n"
        + "\n".join(f"- {dim}" for dim in CLARIFY_DIMENSIONS)
        + "\nReturn ONLY JSON: {\"questions\": [string, ...], \"scope_card\": {"
        + ", ".join(f'"{key}": string' for key in default_scope_card("x"))
        + "}}. Scope card keys must be exactly: "
        + keys
        + "."
    )
    user = f"Topic: {topic}\n\nReturn the JSON object now."
    fallback_questions = list(_FALLBACK_QUESTIONS)
    fallback_card = default_scope_card(topic)
    try:
        raw = await generate_text(system_prompt=system, user_prompt=user, model=model, temperature=0.2)
    except Exception:
        return fallback_questions, fallback_card
    parsed = parse_json_block(raw)
    if isinstance(parsed, list):
        questions = [str(item).strip() for item in parsed if str(item).strip()][:6]
        return (questions or fallback_questions), fallback_card
    if not isinstance(parsed, dict):
        return fallback_questions, fallback_card
    questions = [str(item).strip() for item in parsed.get("questions", []) if str(item).strip()][:6]
    card = _merge_scope_card(topic, parsed.get("scope_card"))
    return (questions or fallback_questions), card


async def build_clarify_questions(
    topic: str, *, generate_text: GenerateText, model: str | None = None
) -> list[str]:
    questions, _card = await build_clarify_draft(topic, generate_text=generate_text, model=model)
    return questions
...
def clarify_pass1_markdown(topic: str, questions: list[str], scope_card: dict[str, str]) -> str:
    ...
    lines = [
        ...
        "以免检索偏离你的真实意图。",
        "- Recommended Default: 采用下方《默认检索范围卡》（已按主题预填，可直接开始检索）。",
