...
- `/fig` uses `IMAGE_MODEL`, which defaults to `gpt-image-2`, only when the user explicitly requests a custom text-free illustration asset. Image generation is never the authoritative source for labels, arrows, or scientific structure.
