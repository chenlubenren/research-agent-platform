...
class ResearchAgentService:
    ...
    async def _run_review_screening(
        self, task: TaskRun, papers: list[PaperRecord], *, topic: str, scope: str, round_label: str
    ) -> dict:
    ...
    # --- Two-round /review helpers (discovery -> direction selection pause -> deep dive) ---
    ...
    async def _run_direction_selection(self, task: TaskRun, stage: StageDefinition, feedback: str):
        ...
        scope_text = (research_brief or topic).strip()
        ...
        bundle = await self.scholar.search_bundle(
            ...
            max_papers=max(24, config.review_direction_max_candidates),
            max_queries=config.review_query_limit,
        ...
        if config.review_screening_enabled:
            screen_summary = await self._run_review_screening(
                task, bundle.papers, topic=topic, scope=scope_text, round_label="探索轮"
            )
        ...
        if config.review_direction_selection_enabled and config.enable_hitl:
            content = build_directions_markdown(directions, recommended, topic=topic, fallback_used=fallback_used)
        else:
    ...
    async def _build_focused_bundle(self, task: TaskRun, topic: str, discovery_queries: list[str]) -> LiteratureBundle:
        ...
        if mode == "panorama" or panorama:
            ...
            focused_queries = focused_query_plan(
                scope_meta, directions, topic=topic, limit=config.review_focused_query_limit
        ...
        if config.review_screening_enabled and working:
            screen_summary = await self._run_review_screening(
                task, working, topic=topic, scope=scope_text, round_label="深挖轮"
            )
            screened = bool(screen_summary.get("screened", False))
        # Order kept papers by screening tier so the capped PDF download prioritizes core work.
        tier_rank = {"core": 0, "transferable": 1, "background": 2, "": 3, "exclude": 9}
        working.sort(
            key=lambda paper: (
        ...
        health = source_health(provider_status)
    ...
    async def _prepare_review_support(self, task: TaskRun, stage: StageDefinition) -> tuple[list, str]:
        # direction_selection is produced deterministically in _execute_stage (first-round
        # retrieval + direction discovery + HITL pause); it needs no LLM prompt support.
        if stage.name == "direction_selection":
            return [], ""
        workspace_root = Path(task.artifact_root)
